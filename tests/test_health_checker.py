...
class TestServiceHealthChecker:
    ...
    def test_check_docker_health_success(self, mock_run_command, health_checker):
        ...
        mock_run_command.side_effect = [
            ...
            MagicMock(returncode=0, stdout="Docker Compose version"),  # docker compose version
            MagicMock(returncode=0, stdout="TYPE     SIZE\nImages   1.2GB"),  # docker system df
            MagicMock(returncode=0, stdout="NETWORK ID   NAME\n123abc   bridge\n456def   media-network")  # docker network ls
        ]
    ...
    def test_check_service_health_success(self, mock_resource_usage, mock_logs,
                                        mock_web_ui, mock_ports, mock_health_status,
                                        mock_running, health_checker, mock_services_config):
    ...
    @patch.object(ServiceHealthChecker, '_is_container_running')
    def test_check_service_health_container_not_running(self, mock_running,
                                                       health_checker, mock_services_config):
    ...
    @patch('src.health_checker.run_command')
    def test_check_container_health_status_healthy(self, mock_run_command, health_checker):
    ...
    @patch('src.health_checker.run_command')
    def test_check_container_health_status_no_healthcheck(self, mock_run_command, health_checker):
    ...
    def test_check_service_ports(self, health_checker, mock_services_config):
        ...
        with patch.object(health_checker, '_test_port_accessibility') as mock_test_port:
            ...
            result = health_checker._check_service_ports("qbittorrent", mock_services_config["qbittorrent"])
    ...
    def test_check_web_ui_health_success(self, mock_urlopen, health_checker, mock_services_config):
        """Test successful web UI health check."""
        mock_response = Mock()
        mock_response.getcode.return_value = 200
        mock_urlopen.return_value = mock_response

        result = health_checker._check_web_ui_health("jellyfin", mock_services_config["jellyfin"])
        assert result is True

    @patch('urllib.request.urlopen')
    def test_check_web_ui_health_auth_redirect(self, mock_urlopen, health_checker, mock_services_config):
    ...
    @patch('urllib.request.urlopen')
    def test_check_web_ui_health_no_url(self, mock_urlopen, health_checker, mock_services_config):
        """Test web UI health check when no setup URL is configured."""
        result = health_checker._check_web_ui_health("gluetun", mock_services_config["gluetun"])
    ...
    @patch('src.health_checker.run_command')
    def test_analyze_container_logs_with_warnings(self, mock_run_command, health_checker):
        ...
        assert "WARNING: Configuration file not found, using defaults" in result["warnings"]

    @patch.object(ServiceHealthChecker, '_is_container_running')
    @patch('src.health_checker.run_command')
    def test_test_vpn_ip_change_success(self, mock_run_command, mock_running, health_checker):
    ...
    @patch.object(ServiceHealthChecker, '_is_container_running')
    @patch('src.health_checker.run_command')
    def test_test_vpn_ip_change_no_change(self, mock_run_command, mock_running, health_checker):
    ...
    def test_check_all_services_success(self, mock_vpn_health, mock_env_vars,
                                      mock_file_perms, mock_network, mock_service_health,
                                      mock_docker_health, health_checker, mock_services_config):
        """Test comprehensive health check with all services healthy."""
        health_checker.load_service_config(mock_services_config)

        # Mock all checks as successful
        mock_docker_health.return_value = {"daemon_running": True, "compose_available": True}
    ...
    def test_determine_overall_status_critical(self, health_checker):
        ...
        results = {
            ...
            "services": {
                "jellyfin": {"container_running": False, "issues": ["Container not running"]},
            },
            "vpn_status": {}
        }
    ...
    def test_determine_overall_status_warning(self, health_checker):
        ...
        results = {
            "docker_health": {"daemon_running": True},
            "services": {
                "jellyfin": {"container_running": True, "issues": ["Warning 1", "Warning 2"]},
                "qbittorrent": {"container_running": True, "issues": ["Warning 3", "Warning 4"]}
            },
            "vpn_status": {"vpn_connected": True}
        }
    ...
    def test_export_health_report_failure(self, mock_open, health_checker, temp_dir, capsys):
